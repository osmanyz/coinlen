...
import TableList from './components/Coins/Table/FirstTableList';
import MobileList from './components/Coins/Mobile/FirstMobileList';
...
import {
  ...
  _checkPreviousPrice,
  _bitcoinOpportunity,
  _checkPreviousPriceArrows,
  _opportunityTextMobile,
  formatKeys,
} from './components/Coins/helpers';

class FirstExchange extends React.Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    document.title = 'Paribu Live Exchange | Coinlen';
  ...
  show(coin) {
    return this.props.history.push(`/exchange/first/${coin}`);
  }
  ...
  render() {
    ...
    return (
      <React.Fragment>
        ...
        <PageTitle currency={currency} time={time} title="Paribu Live Exchange" />
        <React.Fragment>
          {WindowSizes().width > 767 && (
            <TableList
              ...
              _checkPreviousPriceArrows={_checkPreviousPriceArrows}
            />
          )}
          {WindowSizes().width <= 767 && (
            <MobileList
...
export default connect(mapStateToProps, { coinsFormatAction })(FirstExchange);
